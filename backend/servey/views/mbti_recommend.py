...
def getServeyAnswer(request):
    ...
    # mbti결과 가져오기
    mbti_from_joblib = joblib.load('./servey/svm_weight/mbti.pkl')
    ...
    similar_plants = similar_plants[similar_plants['sunlight'].str.contains(str(answer_list[8]))]
    similar_plants = similar_plants[similar_plants['air_purification'] == answer_list[9]]
    similar_plants = similar_plants[similar_plants['humidity'].str.contains(str(answer_list[10]))]
    similar_plants = similar_plants[similar_plants['pet_safe'] == answer_list[11]]
    prefer_plants = prefer_plants[:5]
    similar_plants = pd.concat([similar_plants, prefer_plants], ignore_index=True)
    ...
    sql = "insert into user_detail(user_number, mbti_name, mbti_expression, flower_language, best_partner, best_expression, worst_partner, \
        ...
        recommanded_plant_4_intro, recommanded_plant_5_id,recommanded_plant_5_name,recommanded_plant_5_intro) \
    ...
    curs.execute(sql, (result_json['user_number'], result_json['mbti_name'], result_json['mbti_expression'], result_json['flower_language'],
                       result_json['best_partner'], result_json['best_expression'], result_json['worst_partner'], result_json['worst_expression'],
                       result_json['recommanded_plant_1_id'], result_json['recommanded_plant_1_name'], result_json['recommanded_plant_1_intro'],
                       result_json['recommanded_plant_2_id'], result_json['recommanded_plant_2_name'], result_json['recommanded_plant_2_intro'],
                        result_json['recommanded_plant_3_id'], result_json['recommanded_plant_3_name'], result_json['recommanded_plant_3_intro'],
                        result_json['recommanded_plant_4_id'], result_json['recommanded_plant_4_name'], result_json['recommanded_plant_4_intro'],
                        result_json['recommanded_plant_5_id'],result_json['recommanded_plant_5_name'],result_json['recommanded_plant_5_intro']))


    sql = "UPDATE user SET servey=1 WHERE user_number=%s"
